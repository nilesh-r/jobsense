      ...
      <main className="container mx-auto px-6 py-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-8 space-y-8">
            {/* Tabs Header */}
            <div className="flex p-1 bg-surface rounded-2xl border border-premium w-fit">
                  ...
                  >
                    ...
                    <label 
                      ...
                      className={`group block p-12 rounded-2xl border-2 border-dashed transition-all cursor-pointer text-center
                        ${uploadResumeMutation.isPending ? 'border-primary bg-primary/5' : 'border-premium hover:border-primary/50 hover:bg-foreground/5'}
                      `}
                    >
                      <input
                        type="file"
                      ...
                      {uploadResumeMutation.isPending ? (
                        ...
                        <>
                          <div className="w-16 h-16 mx-auto mb-4 bg-surface rounded-2xl flex items-center justify-center text-muted group-hover:text-primary transition-colors">
                    ...
                    <div className="mt-10">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {resumesLoading ? (
                           [1, 2].map(i => <div key={i} className="h-16 bg-foreground/5 rounded-xl animate-pulse" />)
                        ) : resumes.length === 0 ? (
                           <p className="text-muted text-sm italic">No files uploaded yet.</p>
                        ) : resumes.map(r => (
                          <div key={r.id} className="flex items-center justify-between p-4 rounded-xl bg-foreground/5 border border-premium hover:border-foreground/10 transition-colors">
                    ...
                    <form onSubmit={(e) => {
                      ...
                      <div className="grid md:grid-cols-2 gap-6">
                        <div className="space-y-2">
                          ...
                          <input name="title" required placeholder="e.g. Lead Designer" className="w-full bg-surface border border-premium rounded-xl px-4 py-3 text-foreground focus:outline-none focus:border-primary transition-colors" />
                        </div>
                        <div className="space-y-2">
                          <label className="text-xs font-bold text-muted uppercase ml-1">Company</label>
                          <input name="companyName" required placeholder="e.g. OpenAI" className="w-full bg-surface border border-premium rounded-xl px-4 py-3 text-foreground focus:outline-none focus:border-primary transition-colors" />
                        </div>
                      </div>
                      <div className="space-y-2">
                        <label className="text-xs font-bold text-muted uppercase ml-1">Job Description</label>
                        <textarea name="jdText" required rows={6} placeholder="Paste requirements..." className="w-full bg-surface border border-premium rounded-xl px-4 py-3 text-foreground focus:outline-none focus:border-primary transition-colors resize-none" />
                      </div>
                      <button 
                        type="submit" 
                ...
                {activeTab === 'analyze' && (
                  ...
                  >
                    <div className="flex justify-between items-center mb-8">
                      ...
                      <div className="px-3 py-1 bg-primary/10 rounded-full text-primary text-[10px] font-bold uppercase tracking-widest">AI Core v2.0</div>
                    </div>
                    ...
                    <form onSubmit={(e) => {
                      ...
                      <div className="grid md:grid-cols-2 gap-8">
                        <div className="space-y-4">
                          ...
                          <select name="resumeId" required className="w-full bg-surface border border-premium rounded-2xl px-5 py-4 text-foreground focus:ring-2 focus:ring-primary outline-none appearance-none cursor-pointer shadow-xl transition-all">
                        ...
                        <div className="space-y-4">
                          ...
                          <select name="jobId" required className="w-full bg-surface border border-premium rounded-2xl px-5 py-4 text-foreground focus:ring-2 focus:ring-primary outline-none appearance-none cursor-pointer shadow-xl transition-all">
          ...
          <div className="lg:col-span-4 space-y-8">
            <div className="premium-card spotlight-card p-6">
              ...
              {analysesLoading ? (
                ...
                <div className="text-center py-10 px-4">
                  <div className="w-12 h-12 bg-surface rounded-full flex items-center justify-center mx-auto mb-4 text-muted border border-premium">
                ...
                <div className="space-y-4">
                  {analyses.slice(0, 5).map((a) => (
                    <Link
                      key={a.id}
                      href={`/analysis/${a.id}`}
                      className="block p-4 rounded-2xl bg-surface border border-premium hover:border-primary/30 hover:scale-[1.02] transition-all group shadow-sm"
                    >
                      <div className="flex justify-between items-center">
                        ...
                        <div className="flex flex-col items-center">
                          <span className={`text-xl font-black ${a.atsScore > 75 ? 'text-emerald-500' : a.atsScore > 50 ? 'text-amber-500' : 'text-rose-500'}`}>
                            {a.atsScore}
                          </span>
                        </div>
                      </div>
                  ...
                  {analyses.length > 5 && (
                    <Link href="/analytics" className="block text-center text-sm font-bold text-primary hover:text-primary/80 py-2 transition-colors">
                      View All Reports →
                    </Link>
                  )}
                </div>
            ...
            <div className="premium-card spotlight-card p-6 bg-primary/10 border-primary/20 relative overflow-hidden group">
              ...
              <p className="text-muted text-sm leading-relaxed mb-6">Unlock deep keyword mapping and real-time resume re-writing with Pro.</p>
